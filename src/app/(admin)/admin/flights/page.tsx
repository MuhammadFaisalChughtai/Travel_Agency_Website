...
export default async function AdminFlightsPage({
  ...
  searchParams: { editId?: string; page?: string };
}) {
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Manage flight deals and trending destinations shown on the flights
          page.
        </p>
      </div>
      ...
      <div className="bg-white shadow-sm rounded-xl border border-slate-200 p-8 mb-10">
        ...
        <p className="text-xs text-slate-400 mb-6">
          These appear as destination cards on the public flights page with a
          Load More button.
        </p>
        ...
        {trendingFlights.length === 0 ? (
          <div className="py-12 text-center text-sm text-slate-400">
            No trending destinations yet. Add your first one above.
          </div>
        ) : (
          <div className="max-h-[500px] overflow-y-auto pr-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {trendingFlights.map((tf: any) => (
                <div
                  key={tf.id}
                  className="relative group rounded-2xl overflow-hidden border border-slate-200 shadow-sm h-52 shrink-0"
                >
                  {tf.image && (
                  ...
                  {tf.tag && (
                    <div className="absolute top-3 left-3 z-10">
                      <span className="px-2 py-0.5 rounded-full bg-amber-400 text-[8px] font-black uppercase tracking-wider text-amber-900">
                        {tf.tag}
                      </span>
                    </div>
                  )}
                  <div className="absolute bottom-0 left-0 right-0 p-4 z-10">
                    <p className="text-white font-black text-sm font-heading">
                      {tf.destination}
                    </p>
                    <p className="text-[#eed6c4] text-xs font-medium">
                      From £{tf.price}
                    </p>
                  </div>
                  <form
                  ...
                  >
                    <button
                      type="submit"
                      className="w-7 h-7 rounded-full bg-red-500 text-white text-xs flex items-center justify-center font-bold hover:bg-red-600"
                    >
                      ✕
                    </button>
                  </form>
                ...
                >
                      ...
                      {flight.isReturn && (
                        <div>
                          ...
                          {flight.returnIsTransit ? (
                            <span className="text-amber-700 font-semibold bg-amber-50 px-1.5 py-0.5 rounded">
                              Transit ({flight.returnTransitAirport})
                            </span>
                          ) : (
                            <span className="text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded">
                              Direct
                  ...
                  <td className="px-3 py-4 text-right pr-6">
                    <div className="flex items-center justify-end gap-4">
                      <Link
                        href={`/admin/flights?editId=${flight.id}&page=${page}`}
