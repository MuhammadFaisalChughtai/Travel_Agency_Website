      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {currentBlogs.map((post) => (
            <article
              key={post.id}
              className="bg-white rounded-3xl overflow-hidden border border-slate-200 shadow-sm hover:shadow-xl hover:border-[#6b4f4f]/30 transition-all duration-300 flex flex-col group hover:-translate-y-1 relative"
            >
              ...
              <div className="p-8 flex flex-col flex-grow space-y-4">
                ...
                <div className="border-t border-slate-100 pt-5">
                  <Link
                    prefetch={true}
                    href={`/blog/${post.slug}`}
                    className="inline-flex items-center gap-2 text-xs font-black text-[#6b4f4f] group-hover:text-slate-900 transition-colors duration-300 uppercase tracking-widest relative z-20"
                  >
