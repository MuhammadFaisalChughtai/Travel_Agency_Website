...
export function HajjBlogSection({ blogs }: { blogs: any[] }) {
  ...
  const hajjBlogs = blogs.filter((post: any) => post.category.includes("Hajj") || post.category.includes("Umrah"));

  const handleLoadMore = () => {
    setVisibleCount((prev) => Math.min(prev + 3, hajjBlogs.length));
  };
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {hajjBlogs.slice(0, visibleCount).map((post: any) => (
            <article 
              key={post.id} 
              className="bg-white rounded-3xl overflow-hidden border border-[#eed6c4]/40/25 shadow-[0_10px_35px_rgba(72,52,52,0.03)] hover:shadow-[0_15px_45px_rgba(72,52,52,0.06)] hover:border-[#6b4f4f]/30 transition-all duration-300 flex flex-col group hover:-translate-y-1 relative">
