...
export function HajjBlogSection({ blogs }: { blogs: any[] }) {
  const [visibleCount, setVisibleCount] = useState(3);
  const sectionBlogs = blogs.filter((post: any) => post.category === "Hajj");
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="text-center space-y-3 mb-12">
          ...
          <h2 className="text-2xl md:text-3xl font-heading font-black text-[#483434] tracking-tight">
            Terrific Hajj Journals
          </h2>
          <p className="text-xs md:text-sm text-slate-500 font-light max-w-xl mx-auto leading-relaxed">
            Gain deep insights, tips, and official protocols to prepare mentally, physically, and spiritually for Hajj.
          </p>
        </div>
              ...
              <div className="p-6 flex flex-col flex-grow space-y-4">
                <div className="flex items-center gap-4 text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                  <span className="flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-[#6b4f4f]/70" />
                    {post.date}
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-[#6b4f4f]/70" />
                    {post.readTime}
                  </span>
                </div>

                <h3 className="text-lg font-heading font-black text-[#483434] leading-snug group-hover:text-[#6b4f4f] transition-colors duration-300 line-clamp-2">
                  {post.title}
                </h3>
                ...
                <div className="border-t border-[#eed6c4]/20 pt-4 mt-auto">
                  <Link 
                    href={`/blog/${post.slug}`} 
                    className="inline-flex items-center gap-1.5 text-[11px] font-extrabold text-[#6b4f4f] group-hover:text-[#483434] transition-colors duration-300 uppercase tracking-widest"
                  >
