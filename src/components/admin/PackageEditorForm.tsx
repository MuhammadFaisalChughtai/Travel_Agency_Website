...
export function PackageEditorForm({ initialData }: { initialData?: any }) {
  ...
  const handleAiFill = async (mode: "generate" | "improve" = "generate") => {
    ...
    try {
      const currentData = {
        title, type, stars, price, duration, travelDates, description: content, metaTitle, metaDescription, metaKeywords
      };
      const res = await fetch("/api/admin/generate-content", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "package", prompt: aiPrompt, currentData, keywords: aiKeywords, length: aiLength, mode })
      });
      ...
      console.error(err);
      alert(err.message || `Failed to ${mode} package content.`);
    } finally {
