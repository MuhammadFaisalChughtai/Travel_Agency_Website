...
const ALL_CAROUSEL_SLIDES = [
  ...
  },
  {
    ...
    image:
      "https://images.unsplash.com/photo-1565552645632-d725f8bfc19a?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80",
  },
  {
    id: "umrah-3",
    tabId: "umrah",
    ...
    image:
      "https://images.unsplash.com/photo-1581451556948-2b8e39f37c35?ixlib=rb-4.0.3&auto=format&fit=crop&w=1920&q=80",
  },
];
...
export function HeroSection() {
  ...
  // Auto-play interval
  useEffect(() => {
    if (activeCarouselSlides.length <= 1) return;
    const interval = setInterval(() => {
      setActiveIndex((current) => (current + 1) % activeCarouselSlides.length);
  ...
  const handlePrev = () => {
    setActiveIndex((current) =>
      current === 0 ? activeCarouselSlides.length - 1 : current - 1,
    );
  };
